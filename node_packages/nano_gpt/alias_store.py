...
Non-secret alias settings are stored in ignored local JSON, with read-only
fallback from the legacy package JSON path.
...
_ALIAS_FILE = Path(__file__).with_name("aliases.local.json")
_LEGACY_ALIAS_FILE = Path(__file__).with_name("aliases.json")
...
def _read_alias_map_unlocked() -> Dict[str, Dict[str, Any]]:
    alias_file = _ALIAS_FILE if _ALIAS_FILE.exists() else _LEGACY_ALIAS_FILE
