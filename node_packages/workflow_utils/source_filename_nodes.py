...
from ...utils.pipe_utils import PIPE_CLIP_INDEX, PIPE_MODEL_INDEX, PIPE_VAE_INDEX
...
COMPONENT_INPUT_KEYS = {
    PIPE_MODEL_INDEX: "model",
    PIPE_CLIP_INDEX: "clip",
    PIPE_VAE_INDEX: "vae",
}

LORA_COMPONENT_INPUT_KEYS = {
    PIPE_MODEL_INDEX: "model",
    PIPE_CLIP_INDEX: "clip",
}

PIPE_BUILDER_OUTPUT_COMPONENTS = {
    1: PIPE_MODEL_INDEX,
    2: PIPE_CLIP_INDEX,
    3: PIPE_VAE_INDEX,
}

SAMPLER_OUTPUT_COMPONENTS = {
    2: PIPE_MODEL_INDEX,
    6: PIPE_VAE_INDEX,
    7: PIPE_CLIP_INDEX,
}
...
def _trace_component_from_inputs(
    prompt: Mapping[str, Any] | None,
    inputs: Mapping[str, Any] | None,
    component_index: int,
    visited: set[tuple[str, int]],
    direct_input_keys: Mapping[int, str],
) -> str | None:
    direct_key = direct_input_keys.get(component_index)
    if direct_key is not None:
        direct_filename = _trace_input_filename(prompt, inputs, direct_key, visited)
        if direct_filename is not None:
            return direct_filename

    return _trace_pipe_input_component(prompt, inputs, "pipe", component_index, visited)
...
def _trace_pipe_builder_component(
    ...
    return _trace_component_from_inputs(
        prompt,
        inputs,
        component_index,
        visited,
...
def _trace_filename(
    ...
    try:
        ...
        if class_type == "PipeBuilder":
            component_index = PIPE_BUILDER_OUTPUT_COMPONENTS.get(output_index)
            if component_index is not None:
                return _trace_pipe_builder_component(prompt, inputs, component_index, visited)
            return None

        if class_type in {"PipeKSamplerFull", "PipeHiResFix"}:
            component_index = SAMPLER_OUTPUT_COMPONENTS.get(output_index)
            if component_index is not None:
                return _trace_ksampler_component(prompt, inputs, component_index, visited)
            return None

        if class_type == "LoraStack5":
            component_index = LORA_OUTPUT_COMPONENTS.get(output_index)
            if component_index is not None:
